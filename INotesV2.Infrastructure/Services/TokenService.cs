...
{
    public class TokenService(IConfiguration configuration, IAppDbContext context) : ITokenService
    {
        ...
        public async Task<User> ValidateRefreshToken(Guid UserId, string refresh_Token, CancellationToken cancellationToken = default)
        {
            ...
            if (user == null || user.refresh_token != refresh_Token || user.refresh_token_expiry < DateTime.Now)
            {
                return null!;
            }
            return user;
